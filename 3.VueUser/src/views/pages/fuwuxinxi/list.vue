			...
			<div class="search_view">
				...
				<div class="search_box">
					<el-select
						class="search_sel"
						clearable
						v-model="searchQuery.fuwuleixing" 
						placeholder="服务类型"
						>
						<el-option v-for="item in fuwuleixingLists" :label="item" :value="item"></el-option>
					</el-select>
				</div>
			</div>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="服务项目" :resizable='true' align="left" header-align="left">
...
<script setup>
	...
	const tableDetailClick = (row) => {
		router.push('fuwuxinxiDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
